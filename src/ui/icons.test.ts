...
/**
 * insertAdjacentHTMLで差し込んでいた頃のSVG文字列。DOM生成へ置き換えても
 * 見た目が変わらないことを、12個すべてについて属性と並び順で検証する
 */
const EXPECTED_SVG: Record<string, string> = {
  ...
  FRONT_MATTER: `<svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 20 20"><g fill="none"><path d="M10.32 2.013A4 4 0 0 0 6.162 7.13l-3.987 3.986a.6.6 0 0 0-.176.424V14.4a.6.6 0 0 0 .6.6h2.8a.6.6 0 0 0 .6-.6V13h1.9a.6.6 0 0 0 .6-.6v-1.693l.735-.735a5.51 5.51 0 0 1-.569-.846l-.99.991a.6.6 0 0 0-.176.424V12H5.6a.6.6 0 0 0-.6.6V14H3v-2.293l4.32-4.32l-.118-.303a3.001 3.001 0 0 1 1.96-3.965c.33-.423.72-.796 1.157-1.106zM13.5 6.25a.75.75 0 1 0 0-1.5a.75.75 0 0 0 0 1.5zM9 6.5a4.5 4.5 0 1 1 7 3.742v2.05l.783.784a.6.6 0 0 1 0 .848L15.707 15l1.068 1.067a.6.6 0 0 1-.05.893l-2.35 1.88a.6.6 0 0 1-.75 0l-2.4-1.92a.6.6 0 0 1-.225-.468v-6.21A4.496 4.496 0 0 1 9 6.5zM13.5 3a3.5 3.5 0 0 0-1.75 6.532a.5.5 0 0 1 .25.433v6.295l2 1.6l1.751-1.401l-1.034-1.035a.6.6 0 0 1 0-.848l1.076-1.076l-.617-.617a.6.6 0 0 1-.176-.424V9.965a.5.5 0 0 1 .25-.433A3.5 3.5 0 0 0 13.5 3z" fill="currentColor"></path></g></svg>`,
  PREVIEW: `<svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 20 20"><g fill="none"><path d="M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14c1.1 0 2-.9 2-2V5a2 2 0 0 0-2-2zm0 16H5V7h14v12zm-5.5-6c0 .83-.67 1.5-1.5 1.5s-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5s1.5.67 1.5 1.5zM12 9c-2.73 0-5.06 1.66-6 4c.94 2.34 3.27 4 6 4s5.06-1.66 6-4c-.94-2.34-3.27-4-6-4zm0 6.5a2.5 2.5 0 0 1 0-5a2.5 2.5 0 0 1 0 5z" fill="currentColor"></path></g></svg>`,
  SCORE: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="16" viewBox="0 0 20 20"><g fill="none"><path d="M11 6H9V4h2v2zm4-2h-2v2h2V4zM9 14h2v-2H9v2zm10-4V8h-2v2h2zm0 4v-2h-2v2h2zm-6 0h2v-2h-2v2zm6-10h-2v2h2V4zm-6 4V6h-2v2h2zm-6 2V8h2V6H7V4H5v16h2v-8h2v-2H7zm8 2h2v-2h-2v2zm-4-2v2h2v-2h-2zM9 8v2h2V8H9zm4 2h2V8h-2v2zm2-4v2h2V6h-2z" fill="currentColor"></path></g></svg>`,
  CROSS: `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"></path><path d="m6 6 12 12"></path></svg>`,
};

/** 元のSVG文字列にある要素間の改行・インデントを取り除く */
function normalize(svg: string): string {
  return svg.replace(/>\s+</g, "><");
}

const ICON_FACTORIES: [string, () => SVGElement][] = [
  ["FILE", createFileIcon],
  ["FOLDER", createFolderIcon],
  ["ALIAS", createAliasIcon],
  ["TAG", createTagIcon],
  ["LINK", createLinkIcon],
  ["HEADER", createHeaderIcon],
  ["SEARCH", createSearchIcon],
  ["FILTER", createFilterIcon],
  ["FRONT_MATTER", createFrontMatterIcon],
  ["PREVIEW", createPreviewIcon],
  ["SCORE", createScoreIcon],
  ["CROSS", createCrossIcon],
];

describe("icons", () => {
  test.each(
    ICON_FACTORIES,
  )("%s は置き換え前と同じSVG要素を生成する", (name, create) => {
    expect(serializeIcon(create())).toBe(normalize(EXPECTED_SVG[name]));
  });

  test("すべてのアイコンに期待値が定義されている", () => {
    expect(ICON_FACTORIES.map(([name]) => name).sort()).toEqual(
      Object.keys(EXPECTED_SVG).sort(),
    );
  });
});
